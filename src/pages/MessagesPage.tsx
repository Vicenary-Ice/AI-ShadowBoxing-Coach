    ...
    return (
      <div className="page-enter min-h-screen bg-background flex flex-col pb-20">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
          {(chatMessages[openChat.id] ?? demoMessages).map((m) => (
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => {
                ...
                const newMsg: Message = { id: `m${Date.now()}`, text: message.trim(), sent: true, time: timeStr };
                setChatMessages((prev) => ({
                  ...prev,
                  [openChat.id]: [...(prev[openChat.id] ?? demoMessages), newMsg],
                }));
                setMessage("");
              }}
              className="p-2.5 rounded-full gradient-fire text-primary-foreground"
            >
              <Send size={16} />
            </button>
          </div>
